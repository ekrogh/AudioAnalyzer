	...
	void startRecording(const File& file)
	{
		...
		if (sampleRate > 0)
		{
			...
			if (auto fileStream = std::unique_ptr<FileOutputStream>(file.createOutputStream()))
			{
				...
				if (auto writer = wavFormat.createWriterFor(fileStream.get(), sampleRate, numInputs, bitDepth, {}, 0))
				{
...
//==============================================================================
class AudioRecorderModule final : public Component
{
public:
	AudioRecorderModule(std::shared_ptr<AudioDeviceManager> SADM)
		: sharedAudioDeviceManager(SADM)
	{
		...
		//                                 sharedAudioDeviceManager->initialise (numInputChannels, 2, nullptr, true, {}, nullptr);
		//                             });

		sharedAudioDeviceManager->addAudioCallback(&liveAudioScroller);
		sharedAudioDeviceManager->addAudioCallback(&recorder);

		setSize(500, 500);
	}

	~AudioRecorderModule() override
	{
...
private:
	...
	void startRecording()
	{
		...
		{
			SafePointer<AudioRecorderModule> safeThis(this);
