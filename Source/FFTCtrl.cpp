...
FFTCtrl::FFTCtrl(std::shared_ptr<FFTModule> ptr_module_FFT, std::shared_ptr<AudioDeviceManager> SADM, std::shared_ptr<freqPlotModule> FPM)
	...
	ptrFFTModule(ptr_module_FFT),
	module_freqPlot(FPM),
	ptrSpectrogramComponent(ptr_module_FFT->getPtrSpectrogramComponent())
{
	//[Constructor_pre] You can add your own custom stuff here..
	ptrSpectrogramComponent->registerFFTCtrl
	(
		std::shared_ptr<FFTCtrl>(this)
	);
	ptrFFTModule->registerFFTCtrl
	(
		std::shared_ptr<FFTCtrl>(this)
	);
	//[/Constructor_pre]
	...
	frequency__textEditor.reset(new juce::TextEditor("freqs text editor"));
	addAndMakeVisible(frequency__textEditor.get());
	frequency__textEditor->setTooltip(TRANS("Frequencies"));
	frequency__textEditor->setExplicitFocusOrder(7);
	frequency__textEditor->setMultiLine(true);
	frequency__textEditor->setReturnKeyStartsNewLine(true);
	frequency__textEditor->setReadOnly(false);
	frequency__textEditor->setScrollbarsShown(true);
	frequency__textEditor->setCaretVisible(true);
	frequency__textEditor->setPopupMenuEnabled(true);
	frequency__textEditor->setText(juce::String());

	frequency__textEditor->setBounds(16, 360, 150, 136);

	frequency__label.reset(new juce::Label("freqs label",
		TRANS("Frequences\n")));
	addAndMakeVisible(frequency__label.get());
	frequency__label->setFont(juce::Font(15.00f, juce::Font::plain).withTypefaceStyle("Regular"));
	frequency__label->setJustificationType(juce::Justification::centredLeft);
	frequency__label->setEditable(false, false, false);
	frequency__label->setColour(juce::TextEditor::textColourId, juce::Colours::black);
	frequency__label->setColour(juce::TextEditor::backgroundColourId, juce::Colour(0x00000000));

	frequency__label->setBounds(16, 325, 150, 24);
...
FFTCtrl::~FFTCtrl()
{
	...
	frequency__textEditor = nullptr;
	frequency__label = nullptr;
...
void FFTCtrl::buttonClicked(juce::Button* buttonThatWasClicked)
{
	...
	else if (buttonThatWasClicked == makeSines__textButton.get())
	{
		...
		StringArray theStrings = StringArray::fromLines(frequency__textEditor->getText());
	...
	else if (buttonThatWasClicked == makeSpectrumOfInput__toggleButton.get())
	{
		...
		{
			ptrFFTModule->switchToMicrophoneInput();
		}
